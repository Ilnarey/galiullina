...
function SortOptions({activeSorting,onChange} : SortPalcesProps){
    ...
    function sortinItemClickHandler(type: SortOffer){
        onChange(type);
        setIsOpen(false)
    }
    return(
        <form className="places__sorting" action="#" method="get" onKeyDown={keyDownHandler}>
                ...
                <ul className={classNames({'places__options--opened' : isOpen},'places__options' , 'places__options--custom')}>
                    {Object.values(SortOffersType).map((type) =>(
                        <li key={type} className={classNames({'places__option--active' : type===activeSorting}, 'places__option')} tabIndex={0}
                        onClick={() => sortinItemClickHandler(type as SortOffer)}>
                            {type}
                        </li>
